...
document.addEventListener("DOMContentLoaded",
  async () => {
    ...
    setStatus("Please wait, loading tasks...");
    try {
      const response = await fetch(`${API_BASE}/get.php?stdid=${STUDENT_ID}&key=${API_KEY}`);
      if (!response.ok) {
        throw new Error("no tasks found " + response.status);
      } else {
        const data = await response.json();
        if (data.tasks && data.tasks.length !== undefined) {
  ...
  deleteButton.addEventListener("click", async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error("Failed to delete task " + response.status);
        ...
        if (data.success) {
          li.remove();
          setStatus("Task successfully deleted");
        } else {
